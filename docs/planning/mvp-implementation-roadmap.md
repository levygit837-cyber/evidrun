...
```mermaid
flowchart LR
    W0["WS-00 Integrar Runtime Kernel"] --> W2["WS-20 Artifact access e capture"]
...
- **Codex / WS-00:** concluir e integrar a worktree do Runtime Kernel, modelo real e read tool;
- **Droid / WS-10:** construir a Console Web contra ports e fixtures, sem editar o backend ou os
  DTOs gerados da worktree ativa.

O frontend deve usar capability detection. Se o Kernel ainda nao estiver em `main`, estados de fila
e attempt usam fixtures de contrato e nunca aparecem como backend disponivel em producao.

### Onda 1 — fronteira de dados e confianca

Depois do merge do Kernel, duas worktrees podem avancar com ownership separado:
...
| --- | --- | --- | --- | --- |
| WS-00 | Runtime Kernel integration | authority em `main` | WS-10 | generic skills, graph, replay |
