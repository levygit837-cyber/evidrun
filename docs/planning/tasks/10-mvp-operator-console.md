...
`/demo/bootstrap`, assume duas Runs e apresenta scores fixos. Cinco prototipos React isolados em
`design/operator-console-prototypes` exploram Lab, Projects, Study, Runs e Chat, mas sao research
visual com backend stub. Esta task cria a estrutura de produto que permite authoring, admissao,
execucao e evidence inspection sobre o Kernel ja integrado.

O agente deve trabalhar em worktree propria e construir contra um port de API testavel. Quando um
...
O Runtime Kernel ja esta em `main`. Redescubra os endpoints e DTOs existentes antes de implementar;
nao copie contratos de prototipos nem altere generated files manualmente.
