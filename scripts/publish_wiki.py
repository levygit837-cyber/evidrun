...
def validate_source_commit(
    ...
    """Fail closed when a generated snapshot does not match its publish revision."""
    if expected_commit is None:
        return
    meta_path = source / ".wiki-meta.json"
    if not meta_path.exists():
        raise SystemExit(
            "source commit verification requested but .wiki-meta.json is missing"
        )
    actual_commit = json.loads(meta_path.read_text(encoding="utf-8")).get("commitHash")
    if actual_commit == expected_commit:
        return
    message = (
        "wiki snapshot is stale: .wiki-meta.json records "
        f"{actual_commit!r}, publish revision is {expected_commit!r}"
    )
    if not allow_stale_source:
        raise SystemExit(message + "; pass --allow-stale-source only after manual review")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help="Fail unless .wiki-meta.json records this exact source commit.",
    )
    parser.add_argument(
        "--allow-stale-source",
        action="store_true",
        help="Allow a reviewed historical snapshot to publish despite commit mismatch.",
    )
